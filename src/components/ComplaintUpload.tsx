        ...
        <div className="card-elevate rounded-2xl p-6 text-center">
          ...
          {!dataUrl && (
            <button
              ...
              className="btn-saffron px-6 py-4 rounded-xl font-semibold text-lg inline-flex items-center gap-2"
            >
              📸 {s("uploadPhoto")}
            </button>
          )}
          {dataUrl && (
            <div className="grid md:grid-cols-2 gap-4 text-left">
              <img
                src={dataUrl}
                alt="upload"
                className="rounded-xl border border-navy/10 max-h-80 w-full object-cover"
              />
              <div>
                {loading && (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      <div>
                        ...
                        <span
                          className={`inline-block mt-1 px-3 py-1 rounded-full border font-bold text-xs ${urgencyColor[complaint.urgency]}`}
                        >
                    ...
                    <div>
                      ...
                      <div className="text-sm text-navy">
                        {pickL(complaint.department_en, complaint.department_hi)}
                      </div>
                      <div className="text-xs text-navy/60">
                        {lang === "en" ? complaint.department_hi : complaint.department_en}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase font-bold text-navy/60">
                    ...
                    <div className="rounded-lg bg-cream border border-navy/10 p-3">
                      <div className="text-sm text-navy">
                        {pickL(complaint.description_en, complaint.description_hi)}
                      </div>
                      <div className="text-xs text-navy/60 mt-1">
                        {lang === "en" ? complaint.description_hi : complaint.description_en}
                      </div>
                    </div>
                    {!filedId ? (
                      <button
                        onClick={submitComplaint}
                        className="btn-green px-5 py-3 rounded-xl font-semibold w-full"
                      >
                        {s("submitComplaint")}
                      </button>
                    ) : (
                ...
                {form && (
                  <div>
                    <h4 className="font-bold text-navy">
                      {pickL(form.form_name_en, form.form_name_hi)}
                    </h4>
                    <div className="text-xs text-navy/60 mb-3">
                      {lang === "en" ? form.form_name_hi : form.form_name_en}
                    </div>
                          ...
                          <div key={i} className="rounded-lg border border-navy/10 bg-white">
                            ...
                            >
                              <span className="font-semibold text-navy text-sm">
                                ...
                                <span className="block text-[11px] text-navy/60 font-normal">
                                  {lang === "en" ? f.label_hi : f.label_en}
                                </span>
                              </span>
                              <ChakraIcon
                                size={16}
                                className={`text-saffron transition-transform ${open ? "rotate-90" : ""}`}
                              />
                            </button>
                            {open && (
                              <div className="px-3 pb-3 text-xs space-y-1 animate-float-up">
                                <div className="text-navy">
                                  {pickL(f.explanation_en, f.explanation_hi)}
                                </div>
                                <div className="text-navy/60">
                                  {lang === "en" ? f.explanation_hi : f.explanation_en}
                                </div>
                                <div className="rounded bg-cream px-2 py-1 text-navy/80">
                                  <b>{lang === "en" ? "Example:" : "उदा:"}</b>{" "}
                                  {pickL(f.example_en, f.example_hi)}
                                </div>
                              </div>
                            )}
                          </div>
                ...
                {err && (
                  <div className="rounded bg-red-50 border border-red-200 p-2 text-red-700 text-sm mt-3">
                    {err}
                  </div>
                )}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-float-up">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
            ...
            <button
              ...
              className="mt-4 w-full text-sm text-navy/60"
            >
